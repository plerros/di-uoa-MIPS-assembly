...
initialize_0:	subu	$sp,$sp,4	#store stack pointer
		sw	$ra,0($sp)

		li	$t0,0
		subu	$sp,$sp,4	#store number 0
		sw	$t0,0($sp)
	...
	print_string1:
		...
		li	$t0,0
		subu	$sp,$sp,4	#store number fibonacci(0)
		sw	$t0,0($sp)
...
initialize_n:	subu	$sp,$sp,4	#store stack pointer
		sw	$ra,0($sp)

		lw	$s0,20($sp)
		addi	$s0,$s0,1
		subu	$sp,$sp,4	#store number n
		sw	$s0,0($sp)
	...
	print_string3:
		...
		subu	$sp,$sp,4	#store number fibonacci(1)
		sw	$t0,0($sp)
